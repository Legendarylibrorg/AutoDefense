...
from crypto_helpers import transport_seal, transport_seal_v2
...
async def test_sealed_v3_roundtrip(client, path: str, aad: bytes, payload: dict[str, Any]) -> None:
    env = transport_seal(payload, aad=aad)
...
async def test_sealed_v2_legacy_decrypt_accepted(client) -> None:
    env = transport_seal_v2({"user_input": "legacy", "model_output": "ok"}, aad=b"analyze")
    res = await client.post("/analyze/sealed", json={"sealed": env})
    assert res.status_code == 200
    assert res.json()["sanitized_input"]


async def test_sealed_v3_wrong_aad_rejected(client) -> None:
    env = transport_seal({"user_input": "hello"}, aad=b"scan")
    _assert_unseal_rejected(await client.post("/analyze/sealed", json={"sealed": env}))


async def test_sealed_v3_tampered_ciphertext_rejected(client) -> None:
    env = transport_seal({"user_input": "hello"}, aad=b"analyze")
