...
Outside `AUTODEFENSE_ENVIRONMENT=local`, the API key is **required** at startup. When data encryption is enabled, `AUTODEFENSE_DATA_KEY_B64` is also required outside local. For `production`, `staging`, or `prod`, the scanner HMAC key is required as well. `POST /scan/kernel` is rejected (503) if the scanner HMAC key is unset outside local so ingest cannot be spoofed unsigned.
