...
| 3 | Supply Chain | Out of scope (runtime) | Runtime cannot fully mitigate compromised upstream packages. **Mitigation in this repo:** frozen `backend/uv.lock` + `frontend/package-lock.json`, local quality gate [docs/CI_LOCAL.md](CI_LOCAL.md) (`npm audit`, OSV lockfile scan via `make ci`); review changelogs before deployments. |
